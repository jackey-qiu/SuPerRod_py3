    ...
    for spectra in spectras:
        ...
        for ii in range(mod.parameters.get_len_rows()):#set the fit parameters
            ...
            if each in ['rgh_raxr.setA'+end_tag,'rgh_raxs.setA'+end_tag]:
                ...
                if "RAXR_FIT_MODE=\"MI\"" in mod.get_script() or "RAXR_FIT_MODE=\'MI\'" in mod.get_script():#if it is a model-independent fit
                    [mod.parameters.set_value(ii+i,2,True) for i in range(5)]
                elif "RAXR_FIT_MODE=\"MD\"" in mod.get_script() or "RAXR_FIT_MODE=\'MD\'" in mod.get_script():#if it is a model-dependent fit
        ...
        mod.data.items[get_previous_accumulate_sum(rank/(size/split_jobs),split_jobs,len(mod.data.items)-num_ctr_data_sets)+spectra+num_ctr_data_sets].use=True#set the to-be-used dataset (first RAXR dataset is the 10th dataset)
