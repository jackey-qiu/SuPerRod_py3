...
for pars in par_list:
    ...
    for spectra in spectras:
        ...
        for ii in range(mod.parameters.get_len_rows()):#set the fit parameters
            ...
            if each=='rgh_raxr.setA'+str(rank/(size/split_jobs)*len(spectras)+spectra+1):
        ...
        mod.data.items[(rank/(size/split_jobs))*len(spectras)+9+spectra].use=True#set the to-be-used dataset (first RAXR dataset is the second dataset)
...
if rank==0:
    ...
    for each_mod in mod_sub_set:
        ...
        for ii in range(len(each_mod)):#set the fit parameters
            each=each_mod[ii][0]
            if each=='rgh_raxr.setA'+str(i*len(spectras)+1):start_temp=ii
            else:pass
            if each=='rgh_raxr.setA'+str(i*len(spectras)+spectras[-1]+1):
                end_temp=ii+4
