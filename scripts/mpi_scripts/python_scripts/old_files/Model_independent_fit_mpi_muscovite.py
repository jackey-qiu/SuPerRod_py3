...
for pars in par_list:
    ...
    spectras=find_spectra_number(rank,size,split_jobs,len(mod.data.items)-1)
    for spectra in spectras:
        ...
        for ii in range(mod.parameters.get_len_rows()):#set the fit parameters
            ...
            if each=='rgh_raxs.setA'+str(get_previous_accumulate_sum(rank/(size/split_jobs),split_jobs,len(mod.data.items)-1)+spectra+1):
        ...
        mod.data.items[get_previous_accumulate_sum(rank/(size/split_jobs),split_jobs,len(mod.data.items)-1)+1+spectra].use=True#set the to-be-used dataset (first RAXR dataset is the second dataset)
...
if rank==0:
    ...
    for each_mod in mod_sub_set:
        ...
        spectras=find_spectra_number_2(i,split_jobs=split_jobs,data_sets=len(mod_temp.data.items)-1)
        for ii in range(len(each_mod)):#set the fit parameters
            each=each_mod[ii][0]
            #if each=='rgh_raxs.setA'+str(i*len(spectras)+1):start_temp=ii
            if each=='rgh_raxs.setA'+str(get_previous_accumulate_sum(i,split_jobs,len(mod.data.items)-1)+1):start_temp=ii
            #str(get_previous_accumulate_sum(rank/(size/split_jobs),split_jobs,len(mod.data.items)-1)+spectra+1)

            else:pass
            if each=='rgh_raxs.setA'+str(get_previous_accumulate_sum(i,split_jobs,len(mod.data.items)-1)+spectras[-1]+1):
                end_temp=ii+4
