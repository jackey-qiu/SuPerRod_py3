...
def print_fancy_radical_plot(coors=np.array([[2.24191,3.146286,7.7466057],[0.27709,0.429286,7.7466057],[ 1.768338,1.488916,7.6507866],[ 0.745624,4.21135,7.6507866]]),basis=np.array([5.038,5.434,0]), cluster_range=0.3,sep_range=None,ax_handle=None):
    ...
    for i in range(-3,4):
        for j in range(-3,4):
            for k in range(len(coors)):
                ...
                ax.scatter([each[0]],[each[1]],marker='.',color=colors[k],s=10,alpha=0.4)
    for i in range(len(seperators)):
        sep=seperators[i]
        if sep==5.42:
            ax.add_artist(pyplot.Circle((coors[0][0],coors[0][1]),sep,color='r',fill=False,lw=1,ls=':'))
        else:
        ...
        if angles[i]>np.pi:
            if sep==5.42:
                ax.text(coors[0][0]+sep*np.cos(angles[i])+dx2,coors[0][1]+sep*np.sin(angles[i])+dy2,str(sep)+r'$\AA$',fontsize=10,color='r',va='center',ha='center',rotation=angles[i]*180/np.pi+90)
            else:
            ...
            for x in x_indexs:
                for i in range(N_sites):
                    if grid[tuple(np.array(grid_index)+(x,y))][0][i]:
                        ax.scatter([grid[tuple(np.array(grid_index)+(x,y))][1][i][0]],[grid[tuple(np.array(grid_index)+(x,y))][1][i][1]],marker=markers[i],color=colors[i],s=3)
                        if add_circle:
...
def build_super_lattice(coordinate_container=[[2.24191,3.146286,7.7466057],[0.27709,0.429286,7.7466057],[ 1.768338,1.488916,7.6507866],[ 0.745624,4.21135,7.6507866]],colors=['r','b','g','black'],x_range=[0,20],y_range=[0,20],roll_order=[1,3],direction='r',ax_handle=None):
    ...
    for y in y_indexs:
        for x in x_indexs:
            if direction=='r':#in row direction
                ...
                ax.scatter([coordinate_container[index][0]+x*5.038],[coordinate_container[index][1]+y*5.434],marker='o',color=colors[index],s=3)
...
def bar_plot(coverage=occ_pb_new,colors=['b','r','g'],labels=['Half layer termination','Full layer termination','Mixture of half and full layer termination'],xlabels=dist_pb_new,ax_handle=None):
    ...
    for i in range(len(coverage)):
        ...
        rec=ax.bar(x,np.array(coverage[i])*100,width=0.8/len(coverage),color=colors[i],label=labels[i],align='center',alpha=0.8)
        #p=pyplot.plot(x,np.array(coverage[i])*100+2,'--d'+colors[i],lw=2,markersize=7)
        #ax.legend()
    #l1,=ax.plot([-1,100],[58,58],'b',lw=3)
    l2,=ax.plot([-1,100],[59,59],'r',lw=3)
    ax.annotate(r'Pb coverage=59% from bestfit model',xy=(3.,67))
